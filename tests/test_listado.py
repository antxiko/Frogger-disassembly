...
Ninguna de estas necesita el cartucho: se hacen sobre src/frogger.asm y
src/frogger.notes. Lo que vigilan es que el listado
...
ASM = os.path.join(RAIZ, "src", "frogger.asm")
NOTES = os.path.join(RAIZ, "src", "frogger.notes")
DOCS = os.path.join(RAIZ, "docs")
OTROS_JUEGOS = ("Pitfall", "Temptations", "Stardust", "Ale Hop", "Colt 36",
                "Middle Earth", "Monkey", "F-1 Spirit", "Athletic",
                "Antarctic", "Pippols", "Super Cobra")

# Time Pilot es la excepcion, y a proposito: su reproductor de sonido y el de
# este cartucho son el mismo codigo, medido byte a byte. Se puede nombrar, pero
# SOLO donde se explica eso; en cualquier otro sitio seria un comentario
# prestado que se ha colado.
OTRO_PERMITIDO = "Time Pilot"

ORG, FIN = 0x4000, 0x6000        # 8 KB: este cartucho acaba en 0x5FFF

# Lo que imprime `make sanity`. Van tambien en make_web.py y en los dos README,
# y el test de abajo vigila que no se separen.
CODIGO, DATOS = 4880, 3312

# Direcciones que se citan en la web y que NO son inicio de instruccion, con su
# razon. Sin esta lista el test de direcciones inventadas las daria por falsas.
BYTES_CITADOS = {
    "4137": "el byte que difiere del reproductor de Time Pilot",
    "415d": "idem: el operando de `ld b,0A8h`, la mezcla del PSG",
    "4171": "idem",
    "4186": "los mismos tres, en las direcciones de Time Pilot",
    "41ac": "idem",
    "41c0": "idem",
    "41b3": "el ultimo byte del reproductor, que acaba justo antes de INIT",
    "4202": "su equivalente en Time Pilot",
}
...
class TestListado(unittest.TestCase):
    ...
    def test_solo_se_nombra_otro_cartucho_donde_toca(self):
        """Time Pilot solo puede salir donde se explica el reproductor."""
        fuera = []
        for ln in notas():
            if OTRO_PERMITIDO not in ln or ln.lstrip().startswith("#"):
                continue          # las lineas de comentario del .notes no van ancladas
            m = re.match(r"^[LCBDF] (0x[0-9A-Fa-f]{4})", ln)
            if not m or not (0x4111 <= int(m.group(1), 16) <= 0x41B3):
                fuera.append(ln[:60])
        self.assertEqual(fuera, [], "nombra Time Pilot fuera del reproductor: %s"
                         % " | ".join(fuera))
...
class TestWeb(unittest.TestCase):
    ...
    def test_la_portada_publica_las_cifras_del_arbol(self):
        ...
        self.assertEqual(CODIGO + DATOS, FIN - ORG,
                         "las cifras publicadas no suman los 8192 bytes")
    ...
    def test_las_paginas_no_inventan_direcciones(self):
        ...
        vivas |= set(BYTES_CITADOS)
        # El final de una directiva D es EXCLUSIVO, asi que al citar un rango
        # en la web se nombra su ultimo byte, que es uno menos. Existe y esta
        # declarado: no es una direccion inventada.
        for ln in notas():
            m = re.match(r"^D 0x([0-9A-Fa-f]{4}) 0x([0-9A-Fa-f]{4})", ln)
            if m:
                vivas.add("%04x" % (int(m.group(2), 16) - 1))
        for raiz, _, ficheros in os.walk(DOCS):
            for fn in ficheros:
                if not fn.endswith(".md"):
                    continue
